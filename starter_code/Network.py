...
class MyNetwork(object):
    ...
    def __init__(self, configs):
        ...
        self.network = ResNet(
            ...
            self.configs['resnet_size'],
            self.configs['num_classes'],
            self.configs['first_num_filters'],
        )
    ...
    def build_network(self, inputs, training):
        # print('Network config',self.configs)
        if inputs==None:
            return self.network
        if training:
            self.network.train()
...
class standard_block(nn.Module):
    ...
    def __init__(self, filters, projection_shortcut, strides, first_num_filters) -> None:
        ...
        ### YOUR CODE HERE
        self.conv1 = nn.Conv2d(in_channels=first_num_filters,out_channels=filters, kernel_size=3, stride=1, padding=1, bias=False)
...
class bottleneck_block(nn.Module):
    ...
    def __init__(self, filters, projection_shortcut, strides, first_num_filters) -> None:
        ...
        # Args given above.
        self.bn1 = nn.BatchNorm2d(filters)
        self.conv1 = nn.Conv2d(in_channels=filters,out_channels= int(filters/4), kernel_size=1, stride=1, bias=False)
        self.bn2 = nn.BatchNorm2d(int(filters/4))
        self.conv2 = nn.Conv2d(in_channels=int(filters/4), out_channels=int(filters/4), kernel_size=3,stride=1, padding=1, bias=False)
        self.bn3 = nn.BatchNorm2d(int(filters/4))
        self.conv3 = nn.Conv2d(in_channels=int(filters/4),out_channels= filters, kernel_size=1,stride=1, bias=False)
        self.shortcut = nn.Sequential()

        if projection_shortcut is not None:
            self.bn1 = nn.BatchNorm2d(first_num_filters)
            self.conv1 = nn.Conv2d(in_channels=first_num_filters, out_channels=int(filters/4), kernel_size=1, stride=strides, bias=False)
            self.bn2 = nn.BatchNorm2d(int(filters/4))
            self.conv2 = nn.Conv2d(in_channels=int(filters/4), out_channels=int(filters/4), kernel_size=3,stride=1, padding=1, bias=False)
            self.bn3 = nn.BatchNorm2d(int(filters/4))
            self.conv3 = nn.Conv2d(in_channels=int(filters/4), out_channels=filters, kernel_size=1,stride=1, bias=False)
