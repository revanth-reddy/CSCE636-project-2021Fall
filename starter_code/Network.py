...
class MyNetwork(nn.Module):
    def __init__(self, block, resnet_size):
        ...
        if block is StandardBlock:
            self.convStack1 = self._stack_layer(block, 16, resnet_size, stride=1) # firstlayer doesn't cut dimensions, so stride=1
            ...
            self.convStack1 = self._stack_layer(block, 64, resnet_size, stride=1) # firstlayer doesn't cut dimensions, so stride=1
